...
class World(object):
    ...
    def restart(self):
        ...
        while self.player is None:
            ...
            """
            spawn_point.location.x = -369.98
            spawn_point.location.y = 5.478
            spawn_point.location.z = 2.0
            spawn_point.rotation.roll = 0.0
            spawn_point.rotation.pitch = 0.0
...
def get_straight_trajectory(town_map):
	...
	for i in range(n_point):
		if town_map['y'][i] > 0 and town_map['y'][i] < 20 and town_map['x'][i] > -450 \
		and town_map['x'][i] < FINAL_x:
	...
	plt.plot(start_x, start_y, "g^", label="Start point", markersize=10)
	plt.plot(end_x, end_y, "b*", label="End point", markersize=10)
	plt.grid(True)
	plt.xlim(-400, FINAL_x+10)
	plt.ylim(-5, 35)
	plt.legend(loc="upper right")
	plt.title('Selected lane change trajectory #1')
	plt.ylabel('y [m]')
	plt.xlabel('x [m]')
...
def main():
    ...
    if len(ego_vehicle_data) > 0 and show_result:
    	...
    	v = []
    	delta = []
    	data = {'ego': ego_vehicle_data, 'surr': surr_vehicle_data, 'coll': collision_data}

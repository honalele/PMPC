...
from RiskSensitiveControl import RiskSensitiveController
...
def visualize_driving_preference(args):
	participant_id = args.participant_id
	
	drv_data = pd.DataFrame()
	#drv_data.columns = ['x', 'y', 'z', 'yaw', 'vx', 'vy', 'ax', 'ay', 'throttle', 'steer', 'brake']
	for s_i in range(5):
		for cnt_j in range(4): 
			file_name = '/home/hona/carla/driving_data/drv_p' + str(args.participant_id) + '_s' + str(s_i+1) + '_c' + str(cnt_j+1) + '.csv' 
			if os.path.isfile(file_name):
				df  = pd.read_csv(file_name)
				df = df.fillna(0)
				drv_data = pd.concat([drv_data, df], axis=1)
			else:
				print('{} Not found'.format(file_name))

	#print('Driving_data is here!!!')
	#print(drv_data.head())
	return drv_data


def get_participant_preference(args):
	drv_data = visualize_driving_preference(args)
	drv_describe = drv_data.describe()

	v_r = drv_describe.at['std', 'vy'] 
	v_min = drv_describe.at['min', 'vy'] 
	v_max = drv_describe.at['max', 'vy'] 
	
	a_r = drv_describe.at['std', 'throttle'] 
	a_min = drv_describe.at['min', 'throttle'] 
	a_max =drv_describe.at['max', 'throttle'] 

	b_r = drv_describe.at['std', 'brake'] 
	b_min = drv_describe.at['min', 'brake'] 
	b_max =drv_describe.at['max', 'brake'] 
	
	d_r = drv_describe.at['std', 'steer'] 
	d_min = drv_describe.at['min', 'steer'] 
	d_max = drv_describe.at['max', 'steer'] 

	omega_r = drv_describe.at['std', 'ay'] 
	omega_min = drv_describe.at['min', 'ay'] 
	omega_max = drv_describe.at['max', 'ay'] 

	l_r = drv_describe.at['std', 'ax'] 
	l_min = drv_describe.at['min', 'ax'] 
	l_max = drv_describe.at['max', 'ax'] 

	"""

	preference = pd.DataFrame(data=[v_r, v_min, v_max, a_r, a_min, a_max, b_r, b_min, b_max, d_r, d_min, d_max, omega_r, omega_min, omega_max, l_r, l_min, l_max])
	preference.columns = ['v_r', 'v_min', 'v_max', 'a_r', 'a_min', 'a_max', 'b_r', 'b_min', 'b_max', 'd_r', 'd_min',\
	 'd_max', 'omega_r', 'omega_min', 'omega_max', 'l_r', 'l_min', 'l_max']
	 """
	preference = [v_r, v_min, v_max, a_r, a_min, a_max, b_r, b_min, b_max, \
	d_r, d_min, d_max, omega_r, omega_min, omega_max, l_r, l_min, l_max]
...
def game_loop(args):
    ...
    try:
        ...
        target_x, target_y, target_yaw = load_scenario(args.scenario_id)
        lc_x, lc_y, lc_yaw = generate_lane_change(target_x, target_y)
        hud = HUD(5000, 2020)
        world = World(client.get_world(), hud)
        preference = get_participant_preference(args)
        print(preference)

        #controller = RiskSensitiveController(world, [lc_x, lc_y, lc_yaw], args.autopilot)
...
def main():
    ...
    argparser.add_argument(
        '--participant_id',
        metavar='PARTICIPANTID',
        default=0,
        type=int,
        help='please input the participant id (default is Naren:0)')

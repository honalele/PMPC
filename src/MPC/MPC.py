...
sys.path.append("/Users/narenbao/workspace/second_journal/src/tools/CubicSpline/")
...
Rd = np.diag([0.1, 0.01])  # input difference cost matrix
Q = np.diag([1.0, 1.0, 0.1, 1.5])  # state cost matrix
Qf = Q  # state final matrix
GOAL_DIS = 1.5  # goal distance
STOP_SPEED = 0.5 / 3.6  # stop speed
MAX_TIME = 11.0  # max simulation time
...
MAX_SPEED = 600.0 / 3.6  # maximum speed [m/s]
MIN_SPEED = -60/ 3.6  # minimum speed [m/s]
...
def do_simulation(cx, cy, cyaw, ck, sp, dl, initial_state, map):
    """
    Simulation
    ...
    while MAX_TIME >= time:
        ...
        if show_animation:  # pragma: no cover
            ...
            if ox is not None:
                plt.plot(ox, oy, "xr", label="Predicted")
            plt.plot(cx, cy, "-y", label="Scenario")
            plt.scatter(-map.iloc[:,0],-map.iloc[:,1],color='black',alpha=0.83,s=0.5)
            plt.plot(x, y, "ob", label="Trajectory")
            plt.plot(xref[0, :], xref[1, :], "xk", label="$z_{r}$")
            plt.plot(cx[target_ind], cy[target_ind], "xg", label="$z_{t}$")
            ...
            for i_car in range(len(xs)):
                ...
                plt.text(xs[i_car] - 2, ys[i_car] + 2, "speed:{}[km/h]".format((round(vs[i_car] * 3.6, 2))))
            ...
            plt.title("MPC Time[s]:" + str(round(time, 2))
                      + ", speed[km/h]:" + str(abs(round(state.v * 3.6, 2))))
            #plt.title("Model Predictive Control Simulation")
            #plt.text(-20, 6, "Time[s]:" + str(round(time, 2)))
            plt.text(state.x, state.y+5, "v_ego: {}[km/h]".format(abs(round(state.v * 3.6, 2))))
            plt.pause(0.0001)
            if round(time)==2 or round(time)==4 or round(time)==6 or round(time)==8 or round(time)==10 or round(time)==11:
                plt.savefig('MPC_trajectory_time_'+str(round(time))+'.png')
...
def get_lane_change_course(course, dl):    
    ...
    ax2 = signal.resample(ax_r,10)
    ay2 = signal.resample(ay_r,10)

    print(len(ax))
    #plt.plot(ax,ay)
    plt.scatter(ax_r[0], ay_r[0], label='start', color='yellow', s=100)
    plt.scatter(ax_r[-1], ay_r[-1], label='end', s=100)
    plt.scatter(ax2,ay2,color='red',s=20)
    plt.legend()
    plt.show()
    cx, cy, cyaw, ck, s = cubic_spline_planner.calc_spline_course(
        ax2, ay2, ds=dl)
...
def generateMPC(i_driver, i_scenario, map, course, drv_data_all):

    #generated_profile = 'generated_profile'
    print('Here we generate MPC')
    dl = 2.0  # course tick
    cx, cy, cyaw, ck = get_lane_change_scenario4(course, dl)
    ...
    t, x, y, yaw, v, d, a = do_simulation(
        cx, cy, cyaw, ck, sp, dl, initial_state, map)
    ...
    if show_animation:  # pragma: no cover
        ...
        plt.ylabel("y[m]")
        plt.legend()
        plt.savefig('pmpc_trajectory_3.png')
